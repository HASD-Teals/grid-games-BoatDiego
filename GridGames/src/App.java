...
public class App {
	...
	public static String[] WizChar() {
		...
		System.out.println("Hello " + Traits[0] + " What robes do you wish to wear?");
		boolean ans = true;
		while (ans == true) {
			System.out.println("Note: It must be a one letter char");
			Traits[1] = input.nextLine();
			if (Traits[1].length() != 1) {
	...
	public static void Wizardgame() {
		// GAME SETUP
		String[] Wizchar1 = new String[1];
		int[][] Wizstat1;
		String[] Wizchar2 = new String[1];
		int[][] Wizstat2;
		if (menu() == true) {
			Wizchar1 = WizChar();
			Wizstat1 = WizStats();
		...
		char player1 = Wizchar1[1].charAt(0);
		char player2 = Wizchar2[1].charAt(0);
		char[][] game;
		game = SetGameBoard(25, 25, ' ');
		writeBlock(game, 0, 25, player2);
		writeBlock(game, 25, 0, player1);
		printBoard(game);
		boolean con = true;
		// GAME PROCESS
		while (con == true) {
	...
	// reports latest
	public static void vWin(char arr[][], char symbol, int amount) {
	...
	// reports latest
	public static void hWin(char arr[][], char symbol, int amount) {
